...
for i,x in enumerate(words):
	G.new_vertex_w_id(i)
	G.set_vertex_attribute(i, "label", x)

new_words = []

G.set_vertex_attribute(30, "label", words[30])

# G.set_vertex_style_attribute(0, "color", "#ffff80")
	...
	for s in wn.synsets(x):
		for h in s.hypernyms():
			 ...
			 try:
				 ...
				 G.new_vertex_w_id(j)
				 G.set_vertex_attribute(j, "color", "#ffff80")
			 G.new_edge(i, j)
...
def vertex_callback(v):
  try:
	if v < 90000:
		G.set_vertex_attribute(v, "label", words[v])
	else:

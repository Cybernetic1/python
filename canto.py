...
# *******************************************
# RNN to convert Chinese sequence to Canto sequence
# perhaps it is based on RL with actions
# and perhaps one output's meaning is "copy"

# need the reward for unsupervised learning
# so it must be based on RL

# -- for each query, RNN reads sentence, outputs sentence
# -- sentence needs to be broken into char encoding (for Chinese text)
# -- but the concept of "word" may also be important
# -- we don't know word segmentation, may not want to be involved in it
# -- but does our task implicitly require word segmentation?
# -- or perhaps we can just look up dictionaries to confirm word boundaries?
# -- 
...
#model.add(SimpleRNN(units=64, dropout=0.05, recurrent_dropout=0.35, return_sequences=True, input_shape=[times_steps, GLOVE_SIZE]))
#model.add(SimpleRNN(units=64, dropout=0.05, recurrent_dropout=0.35, return_sequences=False))
model.add(Dense(units=64, input_shape=[times_steps, GLOVE_SIZE]))

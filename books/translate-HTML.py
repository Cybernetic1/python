...
# Translate HTML text using Google Translate web page
...
if len(sys.argv) < 2:
	print("usage: translate-HTML in-file [out-file]")
	exit(0)
elif len(sys.argv) == 2:
	f1 = open(sys.argv[1], 'r')
	path = path.dirname(sys.argv[1])
	outfile = path + '/chingchong.xhtml'
...
for line in f1:
	...
	if p is None:
		f2.write(line)
		continue
	cls = p.get('class')[0]	# class for <p> tag
	if cls == 'CN':
		f2.write(line)
		continue
	if cls == 'CT':
		print('\x1b[31m' + p.text, end='\x1b[0m\n')
	elif cls == 'TextOpener' or cls == 'TextInd':
		print('\x1b[32m' + p.text, end='\x1b[0m\n')
	else:
		print(p.text)
	...
	# **** various cases of <p> classes
	if cls == 'CT':
		result = soup.find('p')
		result.string.replace_with(translated)
		print(result)
		f2.write(str(result) + '\n')
	elif cls == 'CN':
		f2.write(str(p) + '\n')
	elif cls == 'TextOpener' or cls == 'TextInd':
		f2.write("<p class=\"" + cls + "\">")
		for t in tags:
			print(t, str(t))
			f2.write(str(t))
		f2.write(translated + "</p>\n")
	else:
		call(["beep", "-l", "100", "-f", "512"])
		print("Error in this line:")
		print('**** ' + p.text)
		# input("你可以自己手動翻譯, to clipboard....")
		translated = pyperclip.paste()
		f2.write("<p class=\"" + cls + "\">")
		for t in tags:
			print(t, str(t))
